...
for pkg in hauv_gazebo_v2 autopilot_pkg; do
  rm -rf "$WS/src/$pkg"
...
cd "$WS"
colcon build --packages-select hauv_gazebo_v2 autopilot_pkg 2>&1 | tail -4
...
echo -n "      executables: "
ros2 pkg executables hauv_gazebo_v2 2>/dev/null | awk '{print $2}' | tr '\n' ' '
echo ""
for f in urdf/hauv_gazebo_v2.urdf worlds/underwater.world launch/hauv_sim.launch.py; do
  p="$WS/install/hauv_gazebo_v2/share/hauv_gazebo_v2/$f"
...
echo "    ros2 launch hauv_gazebo_v2 hauv_sim.launch.py gui:=false"
