...
DISPLAY=<your-windows-ip>:0 ros2 launch hauv_gazebo hauv_sim.launch.py
```
...
```bash
wsl -d Ubuntu-Foxy
bash "/mnt/c/Users/matma/Desktop/rov_ws - Copilot/src/hauv_gazebo/fly_sim.sh"
...
of every run land in `/tmp/hauv_sim_results/`.
...
Entry points install to `install/hauv_gazebo/bin/` instead of
`lib/hauv_gazebo/`, which breaks `ros2 run` and launch. `wsl_build.sh` copies
